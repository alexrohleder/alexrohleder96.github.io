...
$(function () {
    ...
    var modal_sidebar = modal.find('.modal-body .col-md-4');
    var modal_content = modal.find('.modal-body .col-md-8');

    modal.find('.close').on('click', function () {
        modal.velocity('fadeOut');
    });

    $('#portfolio li').on('click', function (ev) {
        var data = $(this).data();

        modal_title.html(data.name);
        data.url ? modal_sidebar.find('.url').html('<a href="%s">Link de Acesso</a>'.replace('%s', data.url)) : null;
        modal_sidebar.find('.tools').html(data.tools.split(', ').join('<br>'));
        modal_sidebar.find('.tag').html(data.type);
        modal_content.html('<p>%s</p>'.replace('%s', data.desc));
        modal.velocity(modal.is(':visible') ? 'fadeOut' : 'fadeIn');
